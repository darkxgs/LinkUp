...
export function translateCallableError(err: unknown): string {
  ...
  // 429 = ازدحام/حصة CPU على Cloud Run (الطلب رُفض قبل تنفيذ الدالة) — رسالة ودّية بدل الرمز الخام
  if (msg.includes('فشل الاتصال بالسيرفر (429)') || code === 'functions/resource-exhausted') {
    return 'الخوادم مزدحمة حالياً — انتظر لحظات ثم أعد المحاولة';
  }
