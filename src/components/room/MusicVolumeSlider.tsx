 ...
 *
 * أفقي بمقدار فيزيائي ثابت في كل اللغات: اليسار = 0، اليمين = 100 (كالمألوف
 * في أشرطة الصوت). يعتمد إحداثيات النافذة (pageX) بدل locationX التي
...
export function MusicVolumeSlider({
  ...
  const setFromPageX = useCallback(
    (pageX: number) => {
      ...
      if (w <= 0) return;
      // مقدار فيزيائي: اليسار = 0، اليمين = 100 في كل اللغات (كالمألوف في
      // أشرطة الصوت). pageX/measureInWindow إحداثيات شاشة فيزيائية لا
      // تنعكس بـ RTL — فلا نعكس النسبة. (كان `1 - ratio` في العربية يعكس
      // الإيماءة عكس التعبئة المرسومة لأن swapLeftAndRightInRTL مفعّل
      // يقلب left/right للتعبئة/المقبض أصلاً — فبدا السحب مقلوباً.)
      const ratio = (pageX - trackStart.current) / w;
