...
class PlinkoEngine {
    constructor(canvas, options = {}) {
        ...
        this._bindEmbedResize();
    }

    _isEmbedMode() {
        if (typeof document === 'undefined') return false;
        return document.documentElement.classList.contains('linkup-embed')
    ...
    _updateCanvasDisplaySize() {
        ...
        const container = this.canvas.closest('.plinko-canvas-container') || this.canvas.parentElement;
        const board = this.canvas.closest('.plinko-board-wrapper');
        const cw = Math.max(200, container?.clientWidth || board?.clientWidth || window.innerWidth);
        const ch = Math.max(
            160,
            container?.clientHeight
            || board?.clientHeight
            || Math.round((cw / WIDTH) * HEIGHT),
        );
        const scale = Math.min(cw / WIDTH, ch / HEIGHT, 1);
        const displayW = Math.max(200, Math.round(WIDTH * scale));
        const displayH = Math.max(160, Math.round(HEIGHT * scale));
    ...
    _bindEmbedResize() {
        if (!this._isEmbedMode() || typeof ResizeObserver === 'undefined') return;
        const container = this.canvas?.closest('.plinko-board-wrapper')
            || this.canvas?.closest('.plinko-canvas-container')
    ...
    _handleBallFinish(ball) {
        ...
        if (payouts && binIndex >= 0 && binIndex < payouts.length) {
            ...
            this.audioManager.playWin(multiplier);
    ...
    updateSoundEnabled(enabled) {
        if (this.audioManager) {
            this.audioManager.enabled = enabled;
            if (enabled) {
