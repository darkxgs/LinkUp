...
function connect() {
    ws = new WebSocket(window.LinkUpRoomBoot ? window.LinkUpRoomBoot.wsPath('uno') : 'ws://localhost:8080/ws/uno');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.LinkUpRoomBoot) {
        ...
        if (boot.playerName) nameInput.value = boot.playerName;
        if (boot.joinCode) lobbyIdInput.value = boot.joinCode;
        setTimeout(() => {
            if (!ws || ws.readyState !== 1) return;
            const name = nameInput.value.trim() || boot.playerName;
            if (!name) return;
            if (boot.autoJoin || boot.joinCode || boot.autoCreate) {
                sendMessage({ action: 'join', name, lobbyId: boot.joinCode || undefined });
            }
        }, 900);
    }
    });
