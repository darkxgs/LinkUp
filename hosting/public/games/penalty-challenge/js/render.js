...
(function (global) {
  ...
  const KEEPER_IMG = {
    stand: 'js/assets/keeper.svg',
    0: 'js/assets/left.svg',   // dive LEFT
    1: 'js/assets/keeper.svg', // hold CENTER
    2: 'js/assets/right.svg',  // dive RIGHT
  };
  ...
  class Renderer {
    ...
    // ---- HUD -----------------------------------------------------------------
    _renderChallengeAvatars(state) {
      const ch = window.__linkupChallengeRef;
      if (!ch) return;
      const map = { P1: ch.challengerAvatar || '', P2: ch.challengedAvatar || '' };
      ['P1', 'P2'].forEach((p) => {
        const wrap = document.querySelector('[data-hud-avatar="' + p + '"]');
        if (!wrap) return;
        ...
        if (url) {
          ...
          img.onerror = () => {
            ...
            if (fb) {
              ...
              fb.textContent = (state.names[p] || p)[0];
            }
          };
    ...
    _renderHUD(state) {
      ['P1', 'P2'].forEach((p) => {
        ...
        const balVal = bal.querySelector('.hud-balance-val') || bal;
        balVal.textContent = state.getBalance(p);
      ...
      this._renderChallengeAvatars(state);
      this.el.hudPot.textContent = state.pot;
      this.el.roundLabel.textContent = state.suddenDeath
        ? (window.ChallengeI18n ? window.ChallengeI18n.t('suddenDeath') : 'SUDDEN DEATH')
        : (window.ChallengeI18n
          ? window.ChallengeI18n.t('round', { n: Math.floor((state.stats.P1.kicks + state.stats.P2.kicks) / 2) + 1 })
    ...
    _renderPlayPhase(state) {
      ...
      if (showHandoff) {
        this.el.handoffName.textContent = state.names[ck.keeper];
        const tr = (k, v) => (window.ChallengeI18n ? window.ChallengeI18n.t(k, v) : k);
        const title = document.getElementById('handoff-title');
        const sub = document.getElementById('handoff-sub');
        if (title) title.textContent = tr('passDeviceTo');
        if (sub) sub.textContent = tr('handoffSub');
        if (this.el.handoffReady) this.el.handoffReady.textContent = tr('handoffReady');
      }
      ...
      if (state.phase === PHASE.SHOOTER_AIMING) {
        this.el.pickPrompt.textContent = window.ChallengeI18n
          ? window.ChallengeI18n.t('shootPrompt', { name: state.names[ck.shooter] })
          : '🎯 ' + state.names[ck.shooter] + ', pick where to SHOOT';
        this.el.grid.classList.remove('keeper-mode');
        this.el.grid.classList.add('shooter-mode');
      } else if (state.phase === PHASE.KEEPER_DIVING) {
        this.el.pickPrompt.textContent = window.ChallengeI18n
          ? window.ChallengeI18n.t('divePrompt', { name: state.names[ck.keeper] })
          : '🧤 ' + state.names[ck.keeper] + ', pick where to DIVE';
    ...
    _showToast(outcome) {
      const tr = (k) => (window.ChallengeI18n ? window.ChallengeI18n.t(k) : k);
      this.el.toast.classList.remove('hidden', 'goal', 'save');
      void this.el.toast.offsetWidth;
      if (outcome === 'GOAL') {
        this.el.toastText.textContent = tr('goalToast');
        this.el.toast.classList.add('goal');
      } else {
        this.el.toastText.textContent = tr('savedToast');
    ...
    _renderOver(state) {
      ...
      this.el.overTitle.textContent = tr('playerWins', { name: state.names[w] });
      ...
      this.el.overPayout.innerHTML =
        '<div class="payout-win">' + tr('payoutWin', { name: state.names[w], amount: state.payout }) + ' ' + coinImg + '</div>' +
        '<div class="payout-lose">' + tr('payoutLose', { name: state.names[l], amount: state.bet }) + ' ' + coinImg + '</div>';
    }
  }
