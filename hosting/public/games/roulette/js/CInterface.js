function CInterface(){
    ...
    this._init = function(){
        ...
        var oSpriteBg = s_oSpriteLibrary.getSprite('but_bg');
        var oMoneyBg = createBitmap(oSpriteBg);
        oMoneyBg.x = 191;
        oMoneyBg.y = 93;
        s_oStage.addChild(oMoneyBg);
        
        var oMoneyText =  new CTLText(s_oStage, 
                    200, 100, oSpriteBg.width-16, 24, 
        ...
        _oSpinBut = new CSpinBut(1107,641,s_oSpriteLibrary.getSprite('spin_but'),"  "+TEXT_SPIN,FONT1,"#fff",19,s_oStage);
        ...
        _oBetNeighbors = new CTextButton(740,641,s_oSpriteLibrary.getSprite('but_bg'),TEXT_NEIGHBORS,FONT1,"#fff",16,s_oStage);
        _oBetNeighbors.addEventListener(ON_MOUSE_UP, this._onNeighborsPanel, this);
        
        _oBetFinalsBet = new CTextButton(898,641,s_oSpriteLibrary.getSprite('but_bg'),TEXT_FINALSBET,FONT1,"#fff",16,s_oStage);
    ...
    this._initFichesBut = function(){
        ...
        for(var i=0;i<NUM_FICHES;i++){
            ...
            _aFiches[i] = new CFicheBut(i,iCurX,iCurY,oSprite);
    ...
    this.setMoney = function(iMoney){
        if (window.CasinoCoins && window.CasinoBridge && CasinoBridge.isEmbedded()) {
            _oMoneyAmountText.refreshText(CasinoCoins.fmt(iMoney));
        } else {
    ...
    this.setCurBet = function(iCurBet){
        if (window.CasinoCoins && window.CasinoBridge && CasinoBridge.isEmbedded()) {
            _oBetAmountText.refreshText(CasinoCoins.fmt(iCurBet));
        } else {
    ...
    this.refreshMoney = function(iStartMoney, iMoney){
        _oRollingText = new CRollingTextController(_oMoneyAmountText.getText(), null, iStartMoney , parseFloat(iMoney), 1000, EASE_LINEAR,TEXT_CURRENCY);
    };
