function CRouletteSettings(){
    ...
    this._init = function(){
        ...
        _aFicheValues=new Array(2000,5000,10000,15000,20000,25000);
    ...
    this.generateFichesPileByIndex = function(iFichesValue){
            ...
            do{     
                    ...
                    iValueRest=roundDecimal(iValueRest,2);

                    var iDivisionWithPrecision=roundDecimal(iFichesValue/iCurMaxFicheStake,2);
    ...
    this.setFicheValuesFromChips = function(chips){
            if (!chips || !chips.length) return;
            _aFicheValues = chips.slice(0, 6);
            while (_aFicheValues.length < 6) {
                var last = _aFicheValues[_aFicheValues.length - 1];
                _aFicheValues.push(last * 2);
            }
    };
